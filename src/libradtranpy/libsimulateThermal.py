...
def ProcessSimulation(airmass_num,pwv_num,oz_num,press_num,prof_str='us',proc_str='sa',cloudext=0.0, albedo=0.0, altitude_str ="LSST",FLAG_VERBOSE=False,thermal_output="brightness", wl_min_nm=5000, wl_max_nm=20000, molresol="coarse", temperature=273.15):
    """
...
#   Selection of absorption model 
    ...
    for atmosphere in theatmospheres:
        ...
        uvspec.inp["data_files_path"]  =  os.path.join(os.path.expanduser("~"), libradtrandatapath)
                
        uvspec.inp["atmosphere_file"] = os.path.join(libradtrandatapath, 'atmmod/'+atmosphere+'.dat')
